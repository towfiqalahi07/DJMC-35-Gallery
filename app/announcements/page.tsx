          ...
          {events.map((event) => {
            ...
            return (
              <div 
                ...
                className="group relative flex flex-col sm:flex-row overflow-hidden rounded-3xl bg-zinc-900/40 border border-white/10 cursor-pointer hover:border-purple-500/30 transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_8px_30px_rgb(0,0,0,0.12)] hover:shadow-purple-500/10"
              >
                {/* STRICT MEDIA BLOCK: Fixed width on desktop, absolute cover inside */}
                <div className="relative w-full sm:w-[320px] h-[220px] sm:h-auto shrink-0 overflow-hidden bg-zinc-950 border-b sm:border-b-0 sm:border-r border-white/5">
                  {event.cover_photo_url ? (
                    ...
                    <div className="absolute inset-0 bg-gradient-to-br from-purple-900/20 via-zinc-900 to-zinc-950 flex items-center justify-center">
                      <Calendar className="w-16 h-16 text-purple-500/10 transition-transform duration-700 group-hover:scale-110" />
                    </div>
                  )}
                  
                  {/* DATE BADGE: Locked to Top-Left uniformly */}
                  <div className="absolute top-4 left-4 flex flex-col items-center justify-center w-14 h-16 rounded-2xl bg-black/60 backdrop-blur-md border border-white/10 text-white shadow-xl z-10">
                ...
                {/* CONTENT BLOCK: Enforced minimum height and flex distribution */}
                <div className="flex-1 p-6 sm:p-8 flex flex-col justify-between relative min-h-[220px]">
                  <div>
                    <div className="flex justify-between items-start gap-4 mb-4">
                      <span className={`px-3 py-1 rounded-full text-[10px] sm:text-xs font-semibold uppercase tracking-wider ${
                    ...
                    <h3 className="text-xl sm:text-2xl font-bold text-white group-hover:text-purple-300 transition-colors mb-3 line-clamp-2">
                      {title}
                    </h3>
                    
                    <p className="text-zinc-400 text-sm sm:text-base line-clamp-2 leading-relaxed mb-6">
                      {event.description}
                    </p>
                  </div>
                  
                  <div className="flex flex-wrap items-center gap-3 sm:gap-5 text-xs sm:text-sm font-medium text-zinc-500 mt-auto">
                    <div className="flex items-center gap-2 bg-zinc-800/50 px-3 py-2 rounded-xl border border-white/5">
      ...
      {selectedItem && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-zinc-900 border border-white/10 rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto flex flex-col shadow-2xl">
            <div className="sticky top-0 bg-zinc-900/90 backdrop-blur-md border-b border-white/5 p-6 flex items-start justify-between z-20">
              ...
              <button 
                onClick={closeModal}
                className="p-2 rounded-full bg-white/5 hover:bg-white/10 text-zinc-400 hover:text-white transition-colors absolute right-6 top-6 shrink-0"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-6">
              {/* FIXED MODAL IMAGE: Replaced h-64 with aspect-video for perfect 16:9 ratio on all devices */}
              {selectedItem.type === 'event' && selectedItem.cover_photo_url && (
                <div className="w-full aspect-video rounded-2xl overflow-hidden relative mb-6 bg-zinc-950 shrink-0">
                  <img src={selectedItem.cover_photo_url} alt={selectedItem.parsedTitle || selectedItem.title} className="w-full h-full object-cover object-center" />
                </div>
              )}
              {selectedItem.type === 'event' && (
                <div className="flex flex-wrap items-center gap-4 text-sm font-medium">
                  <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-purple-500/10 text-purple-400 border border-purple-500/20">
                  ...
                  {selectedItem.location && (
                    <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-zinc-800/50 text-zinc-300 border border-white/5">
              ...
              {selectedItem.attachment_url && (
                <div className="pt-6 border-t border-white/5">
                  ...
                  <a 
                    ...
                    className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 transition-colors text-sm font-medium border border-blue-500/20"
                  >
                    View Attachment
                  </a>
                </div>
